...
# Crée des variables binaires pour le genre et le statut professionnel
data = pd.get_dummies(data, columns=["Gender", "Working Professional or Student"], drop_first=True)


# Étape 2 : minmaxscaler aux pour depression score
...
# Étape 3 : Appliquer StandardScaler aux Graph Raw Features
graph_features = [ "Depression Score","Total Pressure", "Total Satisfaction", 
...
#standard scaler pour tracer le graphe
...
weights = np.array([0.3, 0.1, 0.1, 0.05, 0.05, 0.2, 0.1, 0.05, 0.05])  # 9 colonnes
weighted_data = normalized_data * weights

from scipy.spatial.distance import pdist, squareform
distance_matrix = squareform(pdist(weighted_data, metric='euclidean'))
...
# Étape 5 : Détection des communautés avec Louvain
...
final_graph_data = final_graph_data.iloc[kept_nodes].copy()  # très important

# ajouter la colonne "Community"
data["Community"] = [filtered_partition[node] for node in kept_nodes]
final_graph_data["Community"] = data["Community"]  # Assurer que final_graph_data a bien la colonne aussi
...
profil_communautes = final_graph_data.groupby("Community")[variables_a_explorer].mean().round(2)
...
# node_colors = ['blue' if node in highlight_nodes["ménage"] else 'gray' for node in G.nodes()]
...
#node_colors = ['blue' if node in highlight_nodes["suicidal thoughts"] else 'gray' for node in G.nodes()]
